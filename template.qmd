---
title: "A Quarto Template for arXiv Preprints with Typst"
subtitle: "Render to PDF quickly with that typical NeurIPS/ICML/ICLR look and feel"
date: today
date-format: long
author:
  - name: Ada Lovelace
...
affiliations:
  ...
  - id: misk
    ...
    department: Department of Quarto Engineering
...
notice: "Preprint. Submitted to the first annual NeurIPS Conference. Under review since 1987."
hide-emails: false
lineno: false
table-stripes: true
format:
...
gt::gt(mtcars[1:8, 1:5], rownames_to_stub = TRUE) |>
  gt::tab_options(
...
# Supplementary Material {#sec-supplement .unnumbered}

Anything after the `#show: appendix` raw block is numbered as an appendix, and the explicit `#pagebreak(weak: true)` above it starts it on a new page (drop that line to let the appendix run on). This heading carries `{.unnumbered}`, so it acts as an unlettered banner over the whole appendix and the lettering starts at the next numbered heading; drop the class if you want the banner itself lettered. Quarto's `@sec-` cross-references only work on numbered headings, so this banner is referenced from the introduction with a plain link, `[supplementary material](#sec-supplement)`.

# Additional Results {#sec-results}

Numbered appendix sections are lettered A, B, and so on. Cross-references keep working: this is @sec-results, and @sec-intro still points back to the introduction.
